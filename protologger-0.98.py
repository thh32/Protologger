...
# Command line use is by running python2.7 Protologger_v0-2.py [16S-file] [Genome-file]


File_16S = sys.argv[1] #User input 16S
genome_file = sys.argv[2] #User input Genome
...
bashCommand = 'mkdir '+ dir_path + 'Input/' + project_name
print bashCommand
...
bashCommand = 'cp ' + genome_file + ' ' + dir_path + 'Input/' + project_name +'/' + sys.argv[2].split('/')[-1:][0].replace('.dat','.fna')
print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()

File16S =  dir_path + 'Input/' + project_name + '/' + sys.argv[1].split('/')[-1:][0]
genome_file = dir_path + 'Input/' + project_name + '/' + sys.argv[2].split('/')[-1:][0].replace('.dat','.fna')
...
bashCommand = 'mkdir '+ dir_path + 'Output/' + project_name
print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
...
bashCommand = 'blastn -query ' + File_16S + ' -db ' + dir_path + 'bin/16S-SILVA/LTP-DB/LTPDB -evalue 0.0000000000000000000000001 -perc_identity 60 -outfmt 6 -out ' + dir_path + 'Output/' + project_name +'/LTP-matches.m8' 
print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
...
for line in open(dir_path + 'bin/16S-SILVA/LTP-DB/LTPs132_SSU_compressed.fasta','r'):
    if line.startswith('>'):
        timber = line.split('-')
...
num = 0
for line in open(dir_path + 'bin/DSMZ-valid-list/DSMZ-August-2019.tab','r'):
    num +=1
    if num >1:
        timber = line.replace('\n','').split('\t')
        if len(timber) > 1:
...
for seq in HTSeq.FastaReader(dir_path + 'bin/16S-SILVA/LTP-DB/LTPs132_SSU_compressed.fasta'):
...
#Run UCHIME
bashCommand = 'usearch -uchime ' + File_16S + ' -db ' + dir_path + 'bin/16S-SILVA/LTP-DB/LTPs132_SSU_compressed.fasta -uchimeout ' + dir_path + 'Output/' + project_name + '/Chimera_check.txt' 
print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
...
ln = 0
for line in open(dir_path + 'bin/GTDB-TK/bac120_metadata_r89.tsv','r'):
    ln +=1
    if ln >1:
        timber = line.replace('\n','').split('\t')
...
for genome in Genomes_for_comparison:
    ...
    for k,v in Representative_genomes.iteritems():
        if genome == v:
            taxa = k
            #print genome , taxa
            bashCommand = 'cp ' + dir_path + 'bin/Genome-database/' + genome + '_genomic.fna.gz ' + dir_path + 'Output/'+project_name+'/Genome_analysis/' + taxa + '--' + genome + '_genomic.fna.gz' 
            #print bashCommand
            process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
            output, error = process.communicate()
...
bashCommand = 'phylophlan -i ' + dir_path + 'Output/'+project_name+'/Genome_Tree -d phylophlan --diversity medium  -f ' + dir_path + 'bin/Phylophlan/supermatrix_aa.cfg --nproc 20 --output_folder ' + dir_path + 'Output/' + project_name
print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
...
bashCommand = 'python3.6 ' + dir_path + 'bin/KEGG-analysis/prokka2kegg.py -i ' + dir_path + 'Output/'+project_name+'/PROKKA-annotation/Isolate.gbk -d ' + dir_path + 'bin/KEGG-analysis/idmapping_KO.tab.gz -o ' + dir_path + 'Output/'+project_name+'/KEGG_profile.KOs' 
print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
...
for line in open(dir_path + 'bin/KEGG-analysis/brite.map','r'):
    timber = line.replace('\n','').split('\t')
                ...
                if 'K01749' in KOs:
                    if 'K01719' in KOs or 'K13542' in KOs or 'K13543' in KOs:
                        if 'K02302' in KOs:
                            ...
                            outputting_overview.write('Siroheme biosynthesis from glutamate was identified from the genome (EC:6.1.1.17, 1.2.1.70, 5.4.3.8, 4.2.1.24, 2.5.1.61, 4.2.1.75, 2.1.1.107/1.3.1.76/4.99.1.4)\n')
...
outputting_overview.write('The following carbon sources were identified to be utilised; ' + ', '.join(carbon_sources) +'\n')
...
if 'K00625' in KOs or 'K13788' in KOs or 'K15024' in KOs:
    if 'K00925' in KOs:
        ...
        outputting_overview.write('Acetate production identified from acetyl-CoA (EC:2.3.1.8, 2.7.2.1)\n')
      
        
if 'K01034' in KOs or 'K01035' in KOs or 'K19709' in KOs:
    #print 'Butanoate production from butanoyl-CoA (EC:2.8.3.8)'
    outputting_overview.write('Butanoate production from butanoyl-CoA (EC:2.8.3.8)\n')
    
if 'K00634' in KOs:
    if 'K00929' in KOs:
        #print 'Butanoate production from butanoyl-CoA (EC:2.3.1.19, 2.7.2.7)'
        outputting_overview.write('Butanoate production from butanoyl-CoA (EC:2.3.1.19, 2.7.2.7)\n')

        
if 'K13788' in KOs or 'K00625' in KOs or 'K15024' in KOs:
    if 'K00925' in KOs:
        #print 'Propanoate production from propanoyl-CoA (EC:2.3.1.8, 2.7.2.1)'
        outputting_overview.write('Propanoate production from propanoyl-CoA (EC:2.3.1.8, 2.7.2.1)\n')
    elif 'K00932' in KOs or 'K19697' in KOs:
        #print 'Propanoate production from propanoyl-CoA (EC:2.3.1.8, 2.7.2.15)'
        outputting_overview.write('Propanoate production from propanoyl-CoA (EC:2.3.1.8, 2.7.2.15)\n')
elif 'K13923' in KOs:
    if 'K00925' in KOs:
        #print 'Propanoate production from propanoyl-CoA (EC:2.3.1.222, 2.7.2.1)'
        outputting_overview.write('Propanoate production from propanoyl-CoA (EC:2.3.1.222, 2.7.2.1)\n')
    elif 'K00932' in KOs or 'K19697' in KOs:
        #print 'Propanoate production from propanoyl-CoA (EC:2.3.1.222, 2.7.2.15)' 
        outputting_overview.write('Propanoate production from propanoyl-CoA (EC:2.3.1.222, 2.7.2.15)\n')
elif 'K01026' in KOs:
    #print 'Propanoate production from propanoyl-CoA (EC:2.8.3.1)' 
    outputting_overview.write('Propanoate production from propanoyl-CoA (EC:2.8.3.1)\n')
elif 'K01905' in KOs:
    #print 'Propanoate production from propanoyl-CoA (EC:6.2.1.13)' 
    outputting_overview.write('Propanoate production from propanoyl-CoA (EC:6.2.1.13)\n')
...
if 'K00640' in KOs or 'K22304' in KOs:
    if 'K01738' in KOs or 'K13034' in KOs or 'K10150' in KOs or 'K17069' in KOs:
        ...
        outputting_overview.write('Sulfide and L-serine utilised to produce L-cysteine and acetate (EC:2.3.1.30, 2.5.1.47)\n')
    elif 'K10150' in KOs:
        #print 'Sulfide and L-serine utilised to produce L-cysteine and acetate (EC:2.3.1.30, 2.5.1.65)'
        outputting_overview.write('Sulfide and L-serine utilised to produce L-cysteine and acetate (EC:2.3.1.30, 2.5.1.65)\n')
    ...
    if 'K13811' in KOs or 'K00955' in KOs or 'K00860' in KOs:
        if 'K00390' in KOs:
            if 'K00380' in KOs or 'K00381' in KOs:
                ...
                outputting_overview.write('Sulfate assimilatory reduction to sulfide identified via the genome (EC:2.7.7.4, 2.7.1.25, 1.8.4.8, 1.8.1.2)\n')
            elif 'K00392' in KOs:
                #print 'Sulfate assimilatory reduction to sulfide identified via the genome (EC:2.7.7.4, 2.7.1.25, 1.8.4.8, 1.8.7.1)'
                outputting_overview.write('Sulfate assimilatory reduction to sulfide identified via the genome (EC:2.7.7.4, 2.7.1.25, 1.8.4.8, 1.8.7.1)\n')
...
if 'K01915' in KOs:
    if 'K00265' in KOs or 'K00266' in KOs or 'K00264' in  KOs or 'K00284' in KOs:
        ...
        outputting_overview.write('L-glutamate production from ammonia was identified via L-glutamine (EC:6.3.1.2, 1.4.1.-)\n')
...
if 'K02586' in KOs or 'K02591' in KOs or 'K02588' in KOs:
    if 'K00531' in KOs:
        if 'K22896' in KOs or 'K22897' in KOs or 'K22899' in KOs:
            ...
            outputting_overview.write('Nitrogen fixation via the production of ammonia identiied (EC:1.18.6.1)\n')
...
if 'K10944' in KOs and 'K10945' in KOs and 'K10946' in KOs:
    if 'K10535' in KOs:
        if 'K00370' in KOs and 'K00371' in KOs:
            ...
            outputting_overview.write('Nitrification identified via conversion of ammonia into nitrate (EC:1.14.99.39, 1.7.2.6, 1.7.5.1)\n')
...
if 'K00652' in KOs:
    if 'K19562' in KOs:
        if 'K01012' in KOs:
            ...
            outputting_overview.write('Biotin biosynthesis from pimeloyl-ACP/CoA identified (EC:2.3.1.47, 2.6.1.62, 6.3.3.3, 2.8.1.6)\n')
        ...
        if 'K01935' in KOs:
            if 'K01012' in KOs:
                ...
                outputting_overview.write('Biotin (vitamin B7) biosynthesis from pimeloyl-ACP/CoA identified (EC:2.3.1.47, 2.6.1.62, 6.3.3.3, 2.8.1.6)\n')
    ...
    if 'K02232' in KOs:
        if 'K02225' in KOs or 'K02227' in KOs:
            if 'K02231' in KOs:
                ...
                outputting_overview.write('Cobalamin (vitamin B12) biosynthesis from cobinamide identified (EC:2.5.1.17, 6.3.5.10, 6.2.1.10, 2.7.1.156)\n')
...
if 'K00287' in KOs or 'K18589' in KOs or 'K19643' in KOs or 'K18590' in KOs or 'K19644' in KOs or 'K18591' in KOs or 'K13998' in KOs:
    ...
    outputting_overview.write('Folate (vitamin B9) biosynthesis from 7,8-dihydrofolate identified (EC:1.5.1.3)\n')
...
if  'K01497' in KOs or 'K14652' in KOs:
            ...
            if 'K14652' in KOs or 'K02858' in KOs:
                if 'K00794' in KOs:
                    if 'K00793' in KOs:
                        ...
                        outputting_overview.write('Riboflavin (vitamina B2) biosynthesis from GTP identified (EC:3.5.4.25, 3.5.4.26, 1.1.1.193, 3.1.3.104, 4.1.99.12, 2.5.1.78, 2.5.1.9, 2.7.1.26, 2.7.7.2)\n')
    elif 'K01498' in KOs:
        if 'K00082' in KOs:
            if 'K22912' in KOs or 'K20860' in KOs or 'K20861' in KOs or 'K20862' in KOs or 'K21063' in KOs or 'K21064' in KOs:
                if 'K14652' in KOs or 'K02858' in KOs:
    ...
    if 'K00794' in KOs:
        if 'K00793' in KOs:
            ...
            outputting_overview.write('Riboflavin (vitamina B2) biosynthesis from D-ribulose-5-phosphate identified (EC:4.1.99.12, 2.5.1.78, 2.5.1.9, 2.7.1.26, 2.7.7.2)\n')
...
if 'K02548' in KOs:
    if 'K03183' in KOs:
        if 'K10106' in KOs:
            if 'K05357' in KOs:
                outputting_overview.write('Menaquinone (vitamin K2) biosynthesis from polyprenyl-diphosphate (EC:2.5.1.74, 2.1.1.163, 4.1.1.90, 1.17.4.4)\n')
...
if 'K23094' in KOs:
    if 'K17872' in KOs:
        if 'K23095' in KOs:
            if 'K10106' in KOs:
                if 'K05357' in KOs:
                    outputting_overview.write('Phylloquinone (vitamin K1) biosynthesis from phytyl-diphosphate (EC:2.5.1.130, 1.6.5.12, 2.1.1.329, 4.1.1.90, 1.17.4.4)\n')
...
bashCommand = 'diamond blastp --query-cover 80.0 --subject-cover 80.0 --id 80.0 -q ' + dir_path + 'Output/'+project_name+'/PROKKA-annotation/Isolate.faa -d ' + dir_path + 'bin/CARD-annotation/CARDDB-DIAMOND -o ' + dir_path + 'Output/'+project_name+'/CARD-annotation/'+project_name+'.CARD.m8' 
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
...
Antibiotic_data = {}
ln = 0
for line in open(dir_path + 'bin/CARD-annotation/aro_categories_index.tsv','r'):
    timber = line.replace('\n','').split('\t')
    ##print timber
    ln +=1
    if ln >1:
...
bashCommand = 'diamond blastp --query-cover 40.0 --subject-cover 40.0 --min-score 100.0 -q ' + dir_path + 'Output/'+project_name+'/PROKKA-annotation/Isolate.faa -d ' + dir_path + 'bin/CAZy-annotation/CAZyDB-DIAMOND -o ' + dir_path + 'Output/'+project_name+'/CAZy-annotation/'+project_name+'.CAZy.m8' 
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
...
bashCommand = 'mash dist ' + dir_path + 'bin/MAG_database/MAG_MASH_all.msh ' + genome_file + ' -t > ' + dir_path + 'Output/' + project_name + '/MAG_MASH.txt'
print bashCommand
...
for line in open(dir_path + 'bin/MAG_database/Meta_data_combined.txt','r'):
    timber = line.replace('\n','').split('\t')
...
for cenv in glob.glob(dir_path + 'bin/IMNGS/FASTA-1000_files/FASTA_files/*.fasta'):
...
with open(dir_path + 'bin/IMNGS/FASTA-1000_files/Abundance_profiles/IMNGS_abundance_values.pickle', 'rb') as handle:
